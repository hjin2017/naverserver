...
<script>
	...
	function hjh_join(){
		...
		var data = {'st_id':input.eq(0).val(),'st_name':input.eq(1).val(),'st_phon':input.eq(2).val(),'pass':input.eq(3).val()};
		...
		if(input.eq(2).val()==''||input.eq(2).val().length!=11)
			alertStr += '전화 번호를 입력 하세요\n';
		
		if(input.eq(3).val()==''||input.eq(3).val().length<8||(input.eq(3).val()!=input.eq(4).val()))
